...
//  根据入口情况处理
for ( var page in config.entry ){
    config.entry[page].unshift('webpack-dev-server/client?' + localPublicPath);
    if(hot === true){// 开启热替换相关设置
      config.entry[page].unshift('webpack/hot/only-dev-server');
    }
}
